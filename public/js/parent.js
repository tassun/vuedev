...
function sendMessageInterface(win) {
	...
	let msg = {type: "storage", moderator: moderator, API_URL: getApiUrl(), BASE_URL: getBaseUrl(), API_TOKEN: getApiToken(), accessorinfo: info};
...
window.onmessage = function(e) {
    ...
    try {
        ...
        if(payload.type=="accessorinfo") {					
            sendMessageInterface(currentNewWindow);
        }
    } catch(ex) { }
...
function openNewWindow() {
    currentNewWindow = window.open("child.html","child_window");
    currentNewWindow.opener = self; 
}
